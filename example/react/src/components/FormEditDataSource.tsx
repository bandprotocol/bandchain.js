...
export default function FormEditDataSource() {
	...
	const { getRootProps, getInputProps } = useDropzone({ onDrop, accept: 'text/x-python-script' })
		...
		<div className="card">
			...
			<div className="preview" style={{ textAlign: 'center' }}>
				<div className="form-input" style={{ marginBottom: '10px' }}>
					<label htmlFor="osid_input" style={{ marginBottom: '8px', display: 'block' }}>Data Source ID:</label>
					<input type="text" id="osid_input" onChange={e => setDsId(e.target.value)} style={{ textAlign: 'center' }} />
				</div>
				<div className="uploadfile" {...getRootProps()}>
					<input {...getInputProps()} />
					{
						dsFileName ? dsFileName : <div className="btn btn-light btn-small">Upload a data source file (.py format)</div>
					}
				</div>
