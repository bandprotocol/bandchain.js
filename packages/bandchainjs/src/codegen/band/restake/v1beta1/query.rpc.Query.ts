...
export class QueryClientImpl implements Query {
  ...
  vaults(request: QueryVaultsRequest = {
    ...
    const promise = this.rpc.request("band.restake.v1beta1.Query", "Vaults", data);
  ...
  vault(request: QueryVaultRequest): Promise<QueryVaultResponse> {
    ...
    const promise = this.rpc.request("band.restake.v1beta1.Query", "Vault", data);
  ...
  locks(request: QueryLocksRequest): Promise<QueryLocksResponse> {
    ...
    const promise = this.rpc.request("band.restake.v1beta1.Query", "Locks", data);
  ...
  lock(request: QueryLockRequest): Promise<QueryLockResponse> {
    ...
    const promise = this.rpc.request("band.restake.v1beta1.Query", "Lock", data);
  ...
  stake(request: QueryStakeRequest): Promise<QueryStakeResponse> {
    ...
    const promise = this.rpc.request("band.restake.v1beta1.Query", "Stake", data);
    return promise.then(data => QueryStakeResponse.decode(new BinaryReader(data)));
  }
  params(request: QueryParamsRequest = {}): Promise<QueryParamsResponse> {
    const data = QueryParamsRequest.encode(request).finish();
    const promise = this.rpc.request("band.restake.v1beta1.Query", "Params", data);
