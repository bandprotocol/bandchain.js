...
export namespace cometbft.p2p.v1 {
    ...
    export class PexAddrs extends pb_1.Message {
        #one_of_decls: number[][] = [];
        constructor(data?: any[] | {
            addrs?: dependency_1.cometbft.p2p.v1.NetAddress[];
        }) {
            super();
        ...
        get addrs() {
            return pb_1.Message.getRepeatedWrapperField(this, dependency_1.cometbft.p2p.v1.NetAddress, 1) as dependency_1.cometbft.p2p.v1.NetAddress[];
        }
        set addrs(value: dependency_1.cometbft.p2p.v1.NetAddress[]) {
            pb_1.Message.setRepeatedWrapperField(this, 1, value);
        }
        static fromObject(data: {
            addrs?: ReturnType<typeof dependency_1.cometbft.p2p.v1.NetAddress.prototype.toObject>[];
        }): PexAddrs {
            const message = new PexAddrs({});
            if (data.addrs != null) {
                message.addrs = data.addrs.map(item => dependency_1.cometbft.p2p.v1.NetAddress.fromObject(item));
            }
            return message;
        }
        toObject() {
            ...
            if (this.addrs != null) {
                data.addrs = this.addrs.map((item: dependency_1.cometbft.p2p.v1.NetAddress) => item.toObject());
            }
            return data;
        }
        serialize(): Uint8Array;
        ...
        serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
            ...
            if (this.addrs.length)
                writer.writeRepeatedMessage(1, this.addrs, (item: dependency_1.cometbft.p2p.v1.NetAddress) => item.serialize(writer));
            if (!w)
                return writer.getResultBuffer();
        }
        static deserialize(bytes: Uint8Array | pb_1.BinaryReader): PexAddrs {
            ...
            while (reader.nextField()) {
                ...
                switch (reader.getFieldNumber()) {
                    case 1:
                        reader.readMessage(message.addrs, () => pb_1.Message.addToRepeatedWrapperField(message, 1, dependency_1.cometbft.p2p.v1.NetAddress.deserialize(reader), dependency_1.cometbft.p2p.v1.NetAddress));
                        break;
                    default: reader.skipField();
                }
            }
